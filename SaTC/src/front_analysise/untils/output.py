...
# @Author  : TT
# @File    : output.py
from front_analysise.modules.parameter import args_set, function_set, upnp_args_set
...
class Output(BaseOutput):
    ...
    def custom_write(self):
        ...
        self.write_cross_function()
        self.write_cross_keyword()
    ...
    def write_cross_keyword(self):
        backend_res = {}
        backend_bins = {}
        args_all = args_set
        if UPNP_ANALYSISE:
            #args_set += upnp_args_set
            args_all = args_set + upnp_args_set
        for args in args_all:
            backend_strs = args.get_backend_str()
            if len(backend_strs) > 0:
                for bstr in backend_strs:
                    if bstr not in backend_res.keys():
                        backend_res[bstr] = {args.name}
                    else:
                        backend_res[bstr].add(args.name)
                    if bstr[1] not in backend_bins.keys():
                        backend_bins[bstr[1]] = {bstr[0]}
                    else:
                        backend_bins[bstr[1]].add(bstr[0])

        np.save(os.path.join(self.simpleoutput, 'Prar_backend_cross.npy'), backend_res)
        with open(os.path.join(self.simpleoutput, "Prar_backend_cross.result"), "w") as f:
            for bstr, kwset in backend_res.items():
                f.write("{}:{}\n".format(bstr, kwset))
        np.save(os.path.join(self.simpleoutput, 'Prar_backend_bin_strs.npy'), backend_bins)
        with open(os.path.join(self.simpleoutput, "Prar_backend_bin_strs.result"), "w") as f:
            for bin, bstrs in backend_bins.items():
                f.write("{}:{}\n".format(bin, len(bstrs)))

    def write_cross_function(self):
        backend_res = {}
        backend_bins = {}
        for function in function_set:
            backend_strs = function.get_backend_str()
            if len(backend_strs) > 0:
                for bstr in backend_strs:
                    if bstr not in backend_res.keys():
                        backend_res[bstr] = {function.name}
                    else:
                        backend_res[bstr].add(function.name)
                    if bstr[1] not in backend_bins.keys():
                        backend_bins[bstr[1]] = {bstr[0]}
                    else:
                        backend_bins[bstr[1]].add(bstr[0])

        np.save(os.path.join(self.simpleoutput, 'API_backend_cross.npy'), backend_res)
        with open(os.path.join(self.simpleoutput, "API_backend_cross.result"), "w") as f:
            for bstr, kwset in backend_res.items():
                f.write("{}:{}\n".format(bstr, kwset))
        np.save(os.path.join(self.simpleoutput, 'API_backend_bin_strs.npy'), backend_bins)
        with open(os.path.join(self.simpleoutput, "API_backend_bin_strs.result"), "w") as f:
            for bin, bstrs in backend_bins.items():
                f.write("{}:{}\n".format(bin, bstrs))
    ...
    def write_upnp_keywords(self, results):
        ...
        with open(os.path.join(self.detailoutput, "upnp_Clustering_result.result"), "w") as f:
            for res in results:
                ...
                f.write("\n")
    #def write_effective_upnp_info(self, keywords):
        with open(os.path.join(self.simpleoutput, "Upnp_keywords_cross.result"), "w") as f:
            for r in self.keywords:
                f.write("{}\n".format(r))
        np.save(os.path.join(self.simpleoutput, 'Upnp_keywords_cross.npy'), self.keywords)
    ...
    def write_back_result(self):
        ...
        with open(os.path.join(self.detailoutput, "Clustering_result_v2.result"), "w") as f:
            for res in self.result:
                ...
                f.write("\n")
        np.save(os.path.join(self.simpleoutput, 'Front_keywords_cross.npy'), self.function|self.keywords)

    def write_effective_info(self, keywords, functions):
        with open(os.path.join(self.simpleoutput, "Front_Prar_cross.result"), "w") as f:
            for r in keywords:
                f.write("{}\n".format(r))
        with open(os.path.join(self.simpleoutput, "Front_API_cross.result"), "w") as f:
            for r in functions:
                f.write("{}\n".format(r))

    def write_hidden_keywords(self, keywords):
        with open(os.path.join(self.simpleoutput, "Front_hidden_keywords.result"), "w") as f:
            for r in keywords:
                f.write("{}\n".format(r))
        np.save(os.path.join(self.simpleoutput, 'Front_hidden_keywords.npy'), keywords)

    def write_annotation_keywords(self, keywords):
        with open(os.path.join(self.simpleoutput, "Front_annotation_keywords.result"), "w") as f:
            for r in keywords:
                f.write("{}\n".format(r))
        np.save(os.path.join(self.simpleoutput, 'Front_annotation_keywords.npy'), keywords)

    def write_form_requests(self, request_list):
        with open(os.path.join(self.simpleoutput, "Front_form_requests.result"), "w") as f:
            for r in request_list:
                f.write("{}\n".format(r))
        np.save(os.path.join(self.simpleoutput, 'Front_form_requests.npy'), request_list)
