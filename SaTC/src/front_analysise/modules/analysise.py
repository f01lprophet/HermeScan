    ...
    def analysise(self, ANALYSIZER):
        ...
        for suffix, parser in ANALYSIZER.items():
            ...
            for file in files:
                ...
                if isinstance(parseobj, HTMLParser):
                    self.hidden_keywords |= parseobj.hidden_keywords
                    self.annotation_keywords |= parseobj.annotation_keywords
                    self.form_request_list += parseobj.form_request_list
...
class BackAnalysise(_BaseAnalysise):
    ...
    def analysise(self):
        ...
        for keyword in args_set:
            keyword.keyword_embd = semantic_similarity.compute_emb(sbert_model, keyword.name)

        for upnp_keyword in upnp_args_set:
            upnp_keyword.keyword_embd = semantic_similarity.compute_emb(sbert_model, upnp_keyword.name)

        for elf in elfs:
            analysis = AnalysisBinary(elf)
            elf_function_res = analysis.find_function(function_set)
            elf_keyword_res = analysis.find_keywords(args_set, sbert_model)
            elf_upnp_keyword_res = analysis.find_keywords(upnp_args_set, sbert_model)
